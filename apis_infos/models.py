...
class ProjectInst(models.Model):
    ...
    def get_next(self):
        ...
        if next:
            return reverse("info:projectinst_detail", kwargs={"pk": next.first().id})
        return False

    def get_prev(self):
        prev = self.__class__.objects.filter(id__lt=self.id).order_by("-id")
        if prev:
            return reverse("info:projectinst_detail", kwargs={"pk": prev.first().id})
        return False
...
class TeamMember(models.Model):
    ...
    def get_next(self):
        next = self.__class__.objects.filter(id__gt=self.id)
        if next:
            return reverse("info:teammember_detail", kwargs={"pk": next.first().id})
        return False

    def get_prev(self):
        prev = self.__class__.objects.filter(id__lt=self.id).order_by("-id")
        if prev:
            return reverse("info:teammember_detail", kwargs={"pk": prev.first().id})
        return False
...
class AboutTheProject(models.Model):
    ...
    def get_next(self):
        next = self.__class__.objects.filter(id__gt=self.id)
        if next:
            return reverse("info:about_detail", kwargs={"pk": next.first().id})
        return False

    def get_prev(self):
        prev = self.__class__.objects.filter(id__lt=self.id).order_by("-id")
        if prev:
            return reverse("info:about_detail", kwargs={"pk": prev.first().id})
        return False
